    ...
    <section id="servicios" className="py-16 sm:py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-gris-600 max-w-2xl mx-auto">
              ...
              <motion.div key={service.id} variants={cardVariants}>
                <Card
                  className={`h-full transition-all duration-300 cursor-pointer ${
                    service.highlight
                      ? 'border-verde-200 hover:border-verde-300 hover:shadow-lg'
                      : 'border-gris-200 hover:border-gris-300 hover:shadow-md'
                  }`}
                  ...
                  <CardContent className="p-5 sm:p-6">
                    ...
                    <div className="flex items-start justify-between gap-4 mb-3">
                      <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-lg flex items-center justify-center shrink-0 ${
                        service.highlight ? 'bg-verde-100' : 'bg-gris-100'
                      }`}>
                        <Icon className={`h-5 w-5 sm:h-6 sm:w-6 ${
                          service.highlight ? 'text-verde-600' : 'text-gris-600'
                        }`} />
                      </div>
                      <button
                    ...
                    <h3 className={`text-lg sm:text-xl font-serif font-semibold mb-2 ${
                      service.highlight ? 'text-verde-800' : 'text-gris-800'
                    }`}>
                      {service.title}
                    </h3>
                    <p className="text-sm sm:text-base text-gris-600 leading-relaxed">
                      {service.description}
                    </p>
                    ...
                    <AnimatePresence>
                      {isExpanded && (
                        ...
                        >
                          <div className="pt-4 mt-4 border-t border-gris-100">
                            <ul className="space-y-2 mb-4">
                              {service.details.map((detail, idx) => (
                                <li key={idx} className="flex items-start gap-2 text-sm text-gris-600">
                                  <CheckCircle className="h-4 w-4 text-verde-500 mt-0.5 shrink-0" />
                                  {detail}
                                </li>
                              ))}
                            </ul>
        ...
        >
          ...
          <Button
            variant="outline"
            className="border-verde-300 text-verde-700 hover:bg-verde-50"
            asChild
          >
